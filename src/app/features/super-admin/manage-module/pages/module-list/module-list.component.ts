...
export class ModuleListComponent {
  ...
  ngOnInit(): void {
    ...
    this.columns = [
      //{ key: 'ModulePageID', title: 'Module Page ID', width: '10%' },
      { key: 'ModuleID', title: 'Module Name', width: '20%' },
      { key: 'PageName', title: 'Page Name', width: '15%' },
      { key: 'Page', title: 'Page', width: '15%' },
      { key: 'Status', title: 'Status', width: '1%' },
      //{ key: 'SortBy', title: 'Sort Order', width: '10%' },
      { key: 'CreatedOn', title: 'Created On', width: '15%' },
      { key: 'CreatedBy', title: 'Created By', width: '15%' },
      { key: 'ModifiedOn', title: 'Modified On', width: '15%' },
      { key: 'ModifiedBy', title: 'Modified By', width: '15%' },
      { key: 'delete', title: 'Action', width: '2%' },
    ];
  ...
  remove(id: any) {
    this.moduleService.deleteModule(id).subscribe((res: any) => {
      window.scrollTo({
        top: 0,
        behavior: 'smooth',
      });
  ...
  navigateToAdd(id: string | number) {
    this.router.navigate([`superadmin/add-module`, id]);
  }
}
