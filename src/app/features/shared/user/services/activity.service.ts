...
import { ElementRef, HostListener, Inject, Injectable, PLATFORM_ID, TemplateRef } from "@angular/core";
import { ActivatedRoute } from "@angular/router";
import { BsModalRef, BsModalService } from "ngx-bootstrap/modal";
import { delay, fromEvent, interval, map, of, ReplaySubject, Subject, Subscription, take } from "rxjs";
...
export class ActivityService {
    ...
    constructor(private modalService: BsModalService, private sessionService: SessionService, @Inject(PLATFORM_ID) platformId: Object, private route: ActivatedRoute) {
        ...
        if (this.isBrowser) {
            if (!this.modalOpen) {
                ...
                if (timeLeft <= 0) {
                    ...
                    this.beforeModalRef = this.modalService.show(this.beforeExpired, { backdrop: 'static', class: 'modal-dialog-centered' });
                    this.modalOpen = true;
                    this.userActive = false;
                    sessionStorage.setItem('inactiveSession', 'true');
                    timeLeft = Math.floor(timeLeft / 1000);

                    this.interval = interval(1000).pipe(take(timeLeft), map(count => timeLeft - count - 1)).subscribe((seconds) => {
    ...
    sessionIsExpired() {
        this.modalService.show(this.expired, { backdrop: 'static', class: 'modal-dialog-centered' });
        this.modalOpen = true;
    }
    getSecondsLeft() {
